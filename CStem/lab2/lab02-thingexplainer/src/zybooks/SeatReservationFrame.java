...
public class SeatReservationFrame extends JFrame implements ActionListener {
    private JFormattedTextField seatNumberToDelete;
    private JButton buttonToDelete;
    ...
    SeatReservationFrame() {
        ...
        deleteLabel = new JLabel("Seat Number to delete:");


        seatNumberToDelete = new JFormattedTextField(NumberFormat.getIntegerInstance());
        seatNumberToDelete.setEditable(true);
        seatNumberToDelete.setValue(0);
        buttonToDelete = new JButton("Delete Reservation");
        buttonToDelete.addActionListener(this);
        ...
        layoutConst.gridx = 4;
        layoutConst.gridy = 2;
        add(seatNumberToDelete, layoutConst);

        layoutConst = new GridBagConstraints();
        layoutConst.insets = new Insets(1, 10, 10, 1);
        layoutConst.fill = GridBagConstraints.HORIZONTAL;
        layoutConst.gridx = 4;
        layoutConst.gridy = 3;
        add(buttonToDelete, layoutConst);
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        int deleteNum;         // seat number to delete

        // Get source of event (2 buttons in GUI)
        JButton sourceEvent = (JButton) event.getSource();

        if(sourceEvent == buttonToDelete){
            deleteNum = ((Number) seatNumberToDelete.getValue()).intValue();

            if(deleteNum >= NUM_SEATS){
                // Show failure dialog
                JOptionPane.showMessageDialog(this, "Seat doesn't exist!");
            }
            else if (seatResArr.get(deleteNum).isEmpty()) {
                // Show failure dialog
                JOptionPane.showMessageDialog(this, "Seat is not empty!");
            } else{
                seatResArr.get(deleteNum).makeEmpty();
